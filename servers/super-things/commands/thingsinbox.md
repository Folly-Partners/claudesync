...
9. **MANDATORY: Log and Learn** - After executing updates, you MUST:
   ...
   ```bash
   echo '{"ts":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'","original":"ORIGINAL_TITLE","suggested":"SUGGESTED_TITLE","final":"FINAL_TITLE","project":"PROJECT_NAME","accepted":true}' >> ~/claudesync/servers/super-things/data/history.jsonl
   ```

   b) **Update patterns.json** if user made corrections:
   - Read current: `cat ~/claudesync/servers/super-things/data/patterns.json`
...
To see past decisions: `cat ~/claudesync/servers/super-things/data/history.jsonl | tail -20`
...
1. **Read last_triage.json**:
   ```bash
   cat ~/claudesync/servers/super-things/data/last_triage.json
   ```
...
4. **Clear the undo file** after successful revert:
   ```bash
   echo '[]' > ~/claudesync/servers/super-things/data/last_triage.json
   ```
...
1. **Load patterns first** - Read `~/claudesync/servers/super-things/data/patterns.json`
