...
**Response structure from search_messages:**
```json
{
  "messages": [{
    "id": "msg_abc123",           // Individual message ID
    "chatID": "chat_xyz789",      // CRITICAL: This is the CONVERSATION ID
    "text": "Hey everyone!",
    "sender": {
      "id": "user_john123",       // NOT for sending - this is who SENT the message
      "name": "John Smith"
    },
    "timestamp": "2024-01-23T10:30:00Z"
  }],
  "chats": [{
    "id": "chat_xyz789",          // Same as message.chatID
    "name": "Team Lunch",
    "type": "group",
    "participants": [...]         // NOT for sending - just metadata
  }]
}
```

**CRITICAL for group chats:** Use `message.chatID` (the conversation ID) for sending. Do NOT use `sender.id` or any participant ID - those would only message one person.

### Step 2: Dedupe, Filter, and Cap

1. **Combine** results from both calls
2. **Dedupe by chatID** - For each unique `message.chatID`, keep only the most recent message
   - The `chatID` is the CONVERSATION identifier (group or 1:1)
   - For group chats, this single ID represents the entire group
   - Store this exact `chatID` value for use in Phase 3 sending
...
**IMPORTANT:** The `chatID` stored here is the conversation ID from `search_messages`. For group chats, sending to this chatID will message ALL participants in the group.
